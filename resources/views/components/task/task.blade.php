...
<div class="w-3/4 bg-white  rounded-lg flex-col flex h-[26rem] gap-y-5 hover:shadow-md gap-y">
        ...
        class="w-full h-[4rem] py-3 border-b border-gray-200 inline-flex items-center justify-between px-8">
                         ...
                         <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"
                          ...
                          <path fill-rule="evenodd"
                            ...
                            <span class="font-semibold text-lg font-roboto">{{$title}}</span>
                        </span>
        <a {{ $attributes }}
    ...
    <div class="flex-col flex px-5 h-[24] overflow-y-auto gap-y-5">
        ...
        @forelse($tasks as $task)
            <div
                class="w-full h-28  flex gap-x-2 bg-gray-50 hover:bg-slate-100 rounded-md animate__animated
                ...
                <div class="w-1/4 h-20 p-3">
                    <div x-data=" {
                                 slides: [
                                @foreach (\App\Livewire\Web\Dashboard\Index::getTaskImage($task->id) as $slide)
                                    {
                                        imgSrc: '{{ $slide['imgSrc'] }}',
                                        imgAlt: '{{ $slide['imgSrc'] }}',
                                    },
                ...
                <a href="{{ route('tasks.upsert', [$task->id]) }}" class="w-4/6 flex-col flex py-1">
                    <div class="h-1/4 inline-flex items-center gap-x-2">
                        ...
                        <span class="text-xs text-gray-600">By {{$task->allocated->name}}</span>
                    </div>

                    <div class="3/4 flex-col flex justify-start items-start ">
                        <div
                            class="text-md font-semibold capitalize">{{\Illuminate\Support\Str::words($task->vname, 5)}}</div>

                        <div class="text-xs line-clamp-1 w-3/4">{!!$task->body!!}</div>
                    </div>
                </a>
            </div>
